...
def execute(filters=None):
    ...
    columns = [
        ...
        {"label": "MSL Trigger Status", "fieldname": "msl_trigger_status", "fieldtype": "Data", "width": 120},
    ...
    FROM (
        SELECT
            ...
            CASE
                WHEN (bin.actual_qty - bin.reserved_stock) <= COALESCE(ir.warehouse_reorder_level, 0)
                THEN 'YES'
                ELSE 'NO'
            END AS msl_trigger_status,
            COALESCE(
                (
