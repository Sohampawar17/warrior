...
def execute(filters=None):
    ...
    where_sql = """si.docstatus = 1 AND IFNULL(si.is_return, 0) = 1 AND si.posting_date BETWEEN %s AND %s"""
    ...
    /* 1) DR row */
    SELECT
        'Credit Note' AS v_type,
        si.posting_date AS date,
        si.customer AS customer_name,
        'Credit Note' AS v_name_type,
        'DR' AS dr_cr,
        NULL AS qty,
        NULL AS item,
        'New ref' AS type_of_ref,
        si.name AS inv_no,
        ABS(si.rounded_total) AS inv_amt,
...
END,
    ...
    SELECT
        ...
        'New ref',
        si.name,
        ABS(IFNULL(si.rounding_adjustment, 0)),
